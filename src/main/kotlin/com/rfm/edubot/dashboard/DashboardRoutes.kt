    ...
    authenticate("dashboard") {
        route("/app/api") {
            get("/me") {
                ...
                call.respond(MeDto(ctx.tenant.dto(), ctx.user?.dto(), modulesFor(ctx.tenant), ctx.principalType))
            }
            get("/overview") {
                val ctx = call.dashboardContext(tenantRepository, dashboardUsers) ?: return@get
...
private fun Route.crmRoutes(mongo: MongoModule, tenantRepository: TenantRepository, dashboardUsers: DashboardUserRepository, appConfig: AppConfig) {
    ...
    route("/crm") {
        get("/clients") {
            val ctx = call.dashboardContext(tenantRepository, dashboardUsers)?.takeIf { it.tenant.agentType == "CRM_V1" } ?: return@get call.respond(HttpStatusCode.Forbidden)
            val deps = tenantDeps(ctx)
            call.respond(deps.clients.search(call.request.queryParameters["q"].orEmpty()).map { it.dto() })
        }
        post("/clients") {
            val ctx = call.dashboardContext(tenantRepository, dashboardUsers)?.takeIf { it.tenant.agentType == "CRM_V1" } ?: return@post call.respond(HttpStatusCode.Forbidden)
            val deps = tenantDeps(ctx)
        ...
        delete("/standard-items/{id}") {
            val ctx = call.dashboardContext(tenantRepository, dashboardUsers)?.takeIf { it.tenant.agentType == "CRM_V1" } ?: return@delete call.respond(HttpStatusCode.Forbidden)
            val deps = tenantDeps(ctx)
        ...
        patch("/invoices/{id}/paid") {
            val ctx = call.dashboardContext(tenantRepository, dashboardUsers)?.takeIf { it.tenant.agentType == "CRM_V1" } ?: return@patch call.respond(HttpStatusCode.Forbidden)
            val deps = tenantDeps(ctx)
